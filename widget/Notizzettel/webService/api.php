...
/**
 * The Notes Resource which allows access to a set of notes or to create a new one
 */
class NotesResource extends Resource {

    /**
     * Handle a GET request for this resource
     * Returns a list of all Notes in the storage. If the field "filter" is
     * given, then a subset of all notes is returned which matches the filter
     * criterias.
     * @param Request request
     * @return Response
     */
     ...
     * Creates a new note and save it to the storage. Generate an id for it and
     * returns it. The body have to contain the note in json format
     * @param Request request
     * @return Response
     */
    function post($request) {
        ...
        $id = NotesService::updateNote('', $request->data, $storage_filename);
...
/**
 * Provides access and operations on specific notes with a given id
 * Operations such as update, delete, and retrieve.
 */
class SingleNoteResource extends Resource {

    /**
     * Handle a GET request for this resource
     * Retrieves the note given by the id of the note and returns it as json
     * @param Request request
     * @return Response
     */
    function get($request) {
     ...
     * Updates the note given by id and returns the id as text/plain
     * @param Request request
     * @return Response
     */
    function put($request) {
        ...
        error_log("Process: " . strval($request->data));
        $id = NotesService::updateNote($this->parameters['id'], $request->data, $storage_filename);
    ...
    /**
     * Handle a POST request for this resource
     * A Wrapper around the limitations of the W3C Widget Framework we are
     * forced to develop in which could not send DELETE and PUT Methods.
     * To delete a post you can also use this post method with parameter
     * "method=delete".
     * All other POST Requests are forwarded to the PUT Request Handler and
     * and do an update of the post.
     * @param Request request
     * @return Response
     */
    function post($request) {
...
/**
 * Handles all Tag related stuff
 */
class TagsResource extends Resource {

    /**
     * Handle a GET request for this resource
     * Returns a json dictionary with all tags and how often they occour in
     * all notes. Usually used for Tag Clouds or other Tag Ranking Methods
     * @param Request request
     * @return Response
     */
    function get($request) {
...
class NotesService {
    ...
    private static function readFileContent($filename) {
        ...
        if ($file == FALSE) {
            ...
            if ($file == FALSE) {
                throw new Exception("Give us permissions to create new files on the Server!");
            }
            fclose($file);
            $file = fopen($filename, "r");
        }
